...
class CoresetLoader:
    ...
    def _fast_compute_H(self):
        ...
        # return H
        with torch.no_grad():
            for batch_data, batch_targets in dataloader:  # Fixed unpacking
                batch_data = batch_data.to(self.device)
                batch_targets = batch_targets.to(self.device)
    ...
    def _calculate_sample_size(self, layer_size, epsilon, R, L):
        ...
        c = 1.0  # Confidence parameter (reduced for practical applications)
        ...
        # return sample_size
        log_term = math.log(self.n)
        denominator = self.epsilon ** 2
        numerator = c * log_term * (self.R ** 2) * (self.L ** 2)
        
        sample_size = int(numerator / denominator)
        return min(sample_size, layer_size)
    ...
    def _build_coreset(self):
        ...
        # Step 1: Initialize by computing H (anchor point loss)
        ...
        # Step 3: Calculate theoretical sample sizes for each layer
        layer_sample_sizes = []
        ...
        # Step 4: Sample points and assign weights
        ...
        #         # Sample points from this layer
        #         selected = np.random.choice(layers[j], sample_size, replace=False)
                
        #         # Calculate normalized weight for this layer
        #         # Weight = (size of layer) / (sample size * total dataset size)
        #         weight = layer_size / (sample_size * self.n)
                
        #         # Store selected indices and weights
        #         all_selected_indices.extend(selected)
        #         all_weights.extend([weight] * sample_size)
        #         total_weight += weight * sample_size
        ...
        for j in range(self.N + 1):
            ...
            if layer_size > 0:
                sample_size = layer_sample_sizes[j]
                selected = np.random.choice(layers[j], sample_size, replace=False)
                weight = layer_size / (sample_size * self.n)  # Original weight calculation
                all_selected_indices.extend(selected)
                all_weights.extend([weight] * sample_size)
        ...
        # Step 5: Load selected data points efficiently
        ...
        # Concatenate batches and move to device
        if all_data:
...
def train_without_coreset(epochs=3, momentum_value=0.0):
    ...
    optimizer = optim.SGD(model.parameters(), lr=1.0, momentum=momentum_value)
...
def train(model, coreset_loader, optimizer):
    ...
    for data, target, weights in coreset_loader:
        ...
        # loss = (individual_losses * weights).sum()* len(coreset_loader.dataset)
        loss = (individual_losses * weights).sum()
        loss.backward()
        optimizer.step()
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch == 1 or epoch % 1 == 0:  # Rebuild every epoch
            coreset_loader = CoresetLoader(
                ...
                R=1.0,
                L=10.0,
                max_coreset_size=coreset_size,  # Optional practical constraint
                batch_size=512,                 # Adjust based on GPU memory
                sampling_ratio=coreset_size/50000
            )
